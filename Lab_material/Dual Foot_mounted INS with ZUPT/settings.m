function [u,heading]=settings(IMU_left, IMU_right, heading_ekf)
...
simdata.range_constraint=1; % metres
simdata.min_rud_sep = 90;    % [samples]

% Constrained Least Squares L2 norm Method
simdata.Centroid_Method='off';
...
% Load the data
%uL=load_dataset();
uL = [IMU_left(1:3,:)*9.8; IMU_left(4:6,:)];
uR = [IMU_right(1:3,:)*9.8; IMU_right(4:6,:)];
...
%clear uR
%clear uL
heading = heading_ekf*pi/180;
% Initial heading [rad]
simdata.init_heading=150*pi/180;
...
simdata.sigma_vel=1e-2*[1 1 1];      %[m/s] 
simdata.sigma_pos=1e-2*[1 1 1];      %[m] 
